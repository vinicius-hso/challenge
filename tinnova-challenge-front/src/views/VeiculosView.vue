    ...
    >
      <va-form id="form-carro" class="flex flex-col gap-6" ref="formRef">
        <va-input
          v-model="veiculo"
          :rules="[
            (value) => (value && value.length > 0) || 'Veículo é obrigatório',
          ]"
          label="Veículo"
        />
        ...
        <va-input
          v-model="cor"
          :rules="[
            (value) => (value && value.length > 0) || 'Cor é obrigatório',
          ]"
          label="Cor"
        />
...
const fetchVeiculos = async () => {
  ...
  veiculos.value = data.map((el) => {
    ...
    el.criado = new Date(el.criado).toLocaleDateString("pt-BR");
    el.atualizado = new Date(el.atualizado).toLocaleDateString("pt-BR");
    return el;
  });
};
...
export default defineComponent({
  ...
  methods: {
    ...
    async deleteItemById(idx) {
      ...
      if (status === 200)
        this.veiculos = [
          ...this.veiculos.slice(0, idx),
          ...this.veiculos.slice(idx + 1),
        ];
    },
    async editItem() {
      if (!isValid) return;
      ...
      veiculoToUpdate.atualizado = new Date().toLocaleDateString("pt-BR");
    ...
    openModalToEditItemById(id) {
      ...
      this.editItem.marca = this.marcas.find(
        (el) => el.nome === this.editItem.marca
      );
